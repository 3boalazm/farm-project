...
function renderBreedingPage(s){
  ...
  var statsHtml=['#مجموعات التكاثر',ar(breedingRecs.length),'bi-collection-fill','var(--orange)','حوامل الآن',ar(pregnant.length),'bi-heart-fill','var(--blue)','ولادات ناجحة',ar(born.length),'bi-check-circle-fill','var(--green)','إجمالي المواليد',ar(born.reduce((t,r)=>t+(+r.offspring_count||0),0)),'bi-people-fill','var(--yellow)','توائم',ar(twins.length),'bi-people-fill','var(--purple)','رجع شياع',ar(returnHeat.length),'bi-arrow-repeat','var(--red)'];
  var statsCards='';
  for(var si=0;si<36;si+=6){
    ...
    else if(si===12){l='ولادات ناجحة';v=ar(born.length);ic='bi-check-circle-fill';c='var(--green)';}
