...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  renderLoading(el);
  try{
    await load_inventory(el, s);
  }catch(e){
    toast('خطأ في التحميل: '+e.message,'error');
    el.innerHTML=`<div class="empty-state"><i class="bi bi-boxes"></i><p>${e.message}</p></div>`;
  }
});

async function load_inventory(el, s){
  el.innerHTML=`<div class="wonder-card text-center py-4">
    <i class="bi bi-boxes accent-text d-block mb-3" style="font-size:2.5rem"></i>
    <h5 class="fw-bold">التغذية والمخزن</h5>
