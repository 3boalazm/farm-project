...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  var totalBirths = getBirths(null).length;
  renderPageHeader(
    '<i class="bi bi-stars" style="color:var(--yellow)"></i> المواليد',
    'إحصائيات ومتابعة حصرية للمواليد — ' + ar(totalBirths) + ' مولود مسجل (منفصل عن القطيع الكبير)',
    can('animals') ?
...
function getBirths(sp) {
  var cutoff = new Date();
  cutoff.setMonth(cutoff.getMonth() - 6); // حيوانات أصغر من 6 أشهر
  var cutoffStr = cutoff.toISOString().slice(0,10);

  return _animals.filter(function(a) {
    if (a.status !== 'alive') return false;
    if (sp && a.species !== sp) return false;

    // 1. محدد كـ "مواليد" صراحةً
    if (a.purpose === 'birth') return true;

    // 2. سلالته "مواليد" (الطريقة اللي يستخدمها الداشبورد)
    if (a.breed === 'مواليد') return true;

    // 3. عنده أم مسجلة (مولود من تقريع)
    if (a.mother_tag) return true;

    // 4. حيوان صغير أضيف حديثاً (أصغر من 6 أشهر) بصرف النظر عن الغرض
    if (a.birth_date && a.birth_date >= cutoffStr) return true;
...
function renderBirthsPage(s) {
  ...
  function isBirthRecord(a) {
    return a.purpose === 'birth' || a.breed === 'مواليد' || a.mother_tag;
  }
  var born = _animals.filter(function(a) {
    return isBirthRecord(a) && a.birth_date && a.birth_date >= yearStart;
  });
  var dead = _animals.filter(function(a) {
    return a.status === 'dead' && isBirthRecord(a) &&
           a.birth_date && a.birth_date >= yearStart;
  });
